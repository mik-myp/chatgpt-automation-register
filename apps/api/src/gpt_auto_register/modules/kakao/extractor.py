"""Kakao Pay / Nicepay 本地支付链接提取引擎。

应用只调用 ``kakao_link`` 并显式传入动态分配的 KR/VN/KR 代理。
上游脚本附带的 Seed 状态辅助函数仅为兼容原实现保留，独立入口已禁用。
"""
...
HTTP_BACKEND = os.environ.get("KAKAO_HTTP_BACKEND", "requests").strip().lower()
...
_PROXY_COUNTRY_SELECTOR_RE = re.compile(
    r"(?i)(?P<name>country|region|res)(?P<separator>[-_=])(?P<value>[a-z]{2}(?:,[a-z]{2})*)"
)
_state_lock = RLock()
_file_lock = RLock()
...
_billing_lock = RLock()
_recent_billing_fingerprints: set[str] = set()
_event_logger: ContextVar[Callable[[str, str], None] | None] = ContextVar(
    "kakao_event_logger", default=None
)
...
def log(message: str, prefix: str = "") -> None:
    ...
    callback = _event_logger.get()
    if callback is not None:
        callback(line, "warning" if prefix else "info")


@contextmanager
def event_logger(callback: Callable[[str, str], None]) -> Iterator[None]:
    """Route extraction logs to the current worker without sharing state across threads."""
    token = _event_logger.set(callback)
    try:
        yield
    finally:
        _event_logger.reset(token)
...
def kakao_proxy_chain(proxy_seed: str) -> tuple[str, str, str]:
    ...
    if not key or any(
        proxy_chain_key(proxy) != key for proxy in (checkout_proxy, promotion_proxy, provider_proxy)
    ):
...
def record_seed_failure(proxy_seed: str, reason: str) -> str:
    ...
    if (
        not proxy_chain_key(proxy_seed)
        or is_account_error(reason)
        or is_checkout_shape_error(reason)
    ):
        return "kept"
    record = seed_record(proxy_seed)
...
def record_role_failure(proxy_seed: str, role: str, reason: str) -> str:
    ...
    if (
        not proxy_chain_key(proxy_seed)
        or is_account_error(reason)
        or (is_checkout_shape_error(reason) and not is_provider_no_method)
    ):
        return "kept"
    record = role_seed_record(proxy_seed, role)
    record["fail"] = int(record.get("fail") or 0) + 1
    ...
    remove_after = env_int("KAKAO_PROXY_REMOVE_AFTER_FAILS", 3, minimum=1, maximum=100)
    if (is_proxy_health_error(reason) or is_provider_no_method) and int(
        record.get("fail") or 0
    ) >= remove_after:
        should_remove = True
    if should_remove:
        record["removed"] = True
...
def random_kakao_billing(
    token: str,
    config: dict[str, Any] | None = None,
) -> dict[str, str]:
    """每次提交生成新的韩国账单组合，避免姓名、地址和邮箱固定复用。"""
    settings = config if isinstance(config, dict) else {}
    configured_email = str(settings.get("email") or "").strip()
    if "@" not in configured_email:
        raise RuntimeError("账单配置缺少来自 token.json user.email 的完整邮箱")
    apartment_probability = max(0.0, min(1.0, float(settings.get("apartment_probability", 0.65))))

    for _ in range(100):
        seed = hashlib.sha256(
            f"{token}:{uuid.uuid4()}:{time.time_ns()}:{os.urandom(8).hex()}".encode()
        ).digest()
        rng = random.Random(seed)
        address = rng.choice(SEOUL_ADDRESS_SEEDS)
        name = f"{rng.choice(KOREAN_FAMILY_NAMES)}{rng.choice(KOREAN_GIVEN_NAMES)}"
        street_number = address["base"] + rng.randrange(address["span"])
        line1 = f"{address['road']} {street_number}"
        line2 = f"{rng.randint(101, 2404)}호" if rng.random() < apartment_probability else ""
        billing = {
            "name": name,
            "email": configured_email,
            "phone": f"+8210{rng.randint(0, 99_999_999):08d}",
            "line1": line1,
            "line2": line2,
            "city": "서울특별시",
        ...
        fingerprint = hashlib.sha256(
            f"{billing['name']}|{line1}|{line2}|{billing['email']}".encode()
        ).hexdigest()
        with _billing_lock:
            if fingerprint in _recent_billing_fingerprints:
                continue
            _recent_billing_fingerprints.add(fingerprint)
            if len(_recent_billing_fingerprints) > 1000:
                _recent_billing_fingerprints.clear()
                _recent_billing_fingerprints.add(fingerprint)
        return billing
    raise RuntimeError("无法生成不重复的 Kakao 账单信息")


def new_session(proxy: str, backend: str | None = None) -> Any:
    register_proxy_for_redaction(proxy)
    selected_backend = str(backend or HTTP_BACKEND or "requests").strip().lower()
    if selected_backend == "curl_cffi":
        if CurlCffiSession is None:
            raise RuntimeError("curl_cffi 未安装，请改用 requests 后端")
...
def extract_ip_country(source: str, payload: dict[str, Any]) -> tuple[str, str]:
    ...
    if source == "myip":
        return str(payload.get("ip") or ""), str(
            payload.get("cc") or payload.get("country") or ""
        ).upper()
    return "", ""
...
def select_verified_proxy(
    ...
    while True:
        ...
        state_text = (
            "已移除" if state == "removed" else ("进入冷却" if state == "cooling" else "保留")
        )
        log(
...
def select_verified_seed(
    ...
    while True:
        ...
        try:
            ...
            log(
                f"Kakao Seed {proxy_label(proxy_seed)} 无法派生，{state_text}: {reason[:180]}",
                "[WARN] ",
            )
            continue
        ...
        log(
            f"Kakao Seed {proxy_label(proxy_seed)} {state_text}: {preflight_error[:180]}", "[WARN] "
        )
...
def promotion_settings(config: dict[str, Any] | None) -> tuple[str, str]:
    settings = config if isinstance(config, dict) else {}
    mode = str(settings.get("promo_mode") or "campaign").strip().lower()
    promo_id = str(settings.get("promo_id") or "plus-1-month-free").strip()
...
def create_checkout(
    ...
    promo_mode, promo_id = promotion_settings(workflow_config)
...
def update_checkout_promotion(
    ...
    if response.status_code >= 400:
        raise RuntimeError(
            f"checkout/update failed {response.status_code}: {response_error(response)}"
        )
    try:
        payload = response.json() or {}
    except (TypeError, ValueError):
    ...
    log(
        f"{PROMOTION_COUNTRY} checkout/update 成功: "
...
def update_kakao_checkout_taxes(
    ...
    if response.status_code >= 400:
        raise RuntimeError(
            f"checkout/taxes failed {response.status_code}: {response_error(response)}"
        )
...
def expected_amount(payload: dict[str, Any]) -> str:
    options = (
        payload.get("elements_options") if isinstance(payload.get("elements_options"), dict) else {}
    )
    if options.get("amount") is not None:
        return str(int(options["amount"]))
    total_summary = (
        payload.get("total_summary") if isinstance(payload.get("total_summary"), dict) else {}
    )
...
def inspect_kakao_init(payload: dict[str, Any], stage: str, *, require_zero: bool) -> str:
    ...
    log(
        f"{stage} Stripe init: amount={amount}; currency={currency}; "
        f"methods={','.join(methods) or 'none'}"
    )
    if "kakao_pay" not in methods or (require_zero and (amount != "0" or currency != "krw")):
        raise RuntimeError(
            f"checkout_not_kakao_trial: stage={stage} amount={amount} "
            f"currency={currency} methods={methods}"
        )
    return amount
...
def stripe_update_kakao_tax_region(
    ...
    log(
        f"{PROVIDER_COUNTRY} Stripe tax_region 同步成功: {billing['city']} {billing['postal_code']}"
    )
...
def kakao_link(
    ...
    *,
    ...
    global TIMEOUT, APPROVE_RETRY_MAX, POLL_TIMEOUT
    steps = (
        workflow_config.get("steps", {})
        if isinstance(workflow_config, dict) and isinstance(workflow_config.get("steps"), dict)
        else {}
    )

    def step_config(name: str) -> dict[str, Any]:
        value = steps.get(name, {})
        return value if isinstance(value, dict) else {}

    def enter_step(name: str, label: str) -> dict[str, Any]:
        global TIMEOUT
        config = step_config(name)
        TIMEOUT = max(3, min(300, int(config.get("timeout") or TIMEOUT)))
        order = int(config.get("order") or 0)
        country = str(
            config.get("country")
            or role_country("promotion" if config.get("proxy_role") == "vn" else "provider")
        ).upper()
        log(f"[{order or '-'}/9][{country}] {label} (timeout={TIMEOUT}s)")
        return config

    ensure_running(stop_event)
    checkout_backend = str(step_config("token_check").get("http_backend") or HTTP_BACKEND)
    promotion_backend = str(step_config("promotion_update").get("http_backend") or HTTP_BACKEND)
    provider_backend = str(step_config("provider_refresh").get("http_backend") or HTTP_BACKEND)
    checkout_session = new_session(checkout_proxy, checkout_backend)
    promotion_session = new_session(promotion_proxy, promotion_backend)
    provider_session = new_session(provider_proxy, provider_backend)

    token_step = enter_step("token_check", "校验 ChatGPT Token")
    ...
    token_attempts = max(1, min(10, int(token_step.get("attempts") or 3)))
    me = None
    last_me_error: Exception | None = None
    for token_attempt in range(1, token_attempts + 1):
        ensure_running(stop_event)
        if token_attempt > 1:
            ...
            checkout_session = new_session(checkout_proxy, checkout_backend)
            log(
                f"[1/9][KR] TLS/代理传输失败，保持同一 Lsid 重建 Session "
                f"{token_attempt}/{token_attempts}",
                "[WARN] ",
            )
        checkout_session.headers.update(chatgpt_headers)
        try:
            ...
            last_me_error = exc
            if token_attempt < token_attempts and is_proxy_health_error(str(exc)):
                time.sleep(min(token_attempt, 2))
                continue
            raise
    if me is None:
        raise RuntimeError(str(last_me_error or "ChatGPT /me request failed"))
    if me.status_code != 200:
        raise RuntimeError(f"ChatGPT /me failed {me.status_code}: {response_error(me, 500)}")

    ensure_running(stop_event)
    enter_step("checkout_create", "创建 KRW Kakao trial checkout")
    checkout_id, publishable_key, checkout = create_checkout(
        checkout_session, token, workflow_config
    )
    checkout_page = activate_stripe_checkout(checkout_session, checkout_id)

    enter_step("stripe_bootstrap", "Bootstrap Stripe init")
    bootstrap_payload, _ = stripe_init(
    ...
    ensure_running(stop_event)
    enter_step("promotion_update", "checkout/update promotion")
    update_checkout_promotion(promotion_session, token, checkout_id, checkout, workflow_config)

    ensure_running(stop_event)
    enter_step("provider_refresh", "promotion 后刷新 Stripe")
    init_payload, stripe_js_id = stripe_init(
    ...
    billing_config = workflow_config.get("billing", {}) if isinstance(workflow_config, dict) else {}
    billing = random_kakao_billing(token, billing_config)
    log(
        "本次账单已重新生成: "
        f"name={billing['name']}; city={billing['city']}; "
        f"postal={billing['postal_code']}; email=***@{billing['email'].split('@')[-1]}"
    )
    tax_elements_session_id = f"elements_session_{uuid.uuid4().hex[:11]}"
    ensure_running(stop_event)
    enter_step("taxes", "同步 checkout/taxes 与 Stripe tax_region")
    ...
    ensure_running(stop_event)
    enter_step("payment_confirm", "Kakao payment method / pre_confirm / confirm")
    ...
    if pre_confirm.status_code != 200:
        raise RuntimeError(
            f"pre_confirm failed {pre_confirm.status_code}: {response_error(pre_confirm)}"
        )
    ...
    if payment_method_response.status_code != 200:
        raise RuntimeError(
            ...
            f"{response_error(payment_method_response, 1000)}"
        )
    ...
    if confirm_response.status_code != 200:
        raise RuntimeError(
            f"confirm failed {confirm_response.status_code}: "
            f"{response_error(confirm_response, 1000)}"
        )
    ...
    if not redirect and (
        ...
        approve_config = enter_step("approve", "OpenAI approve")
        APPROVE_RETRY_MAX = max(
            1, min(10, int(approve_config.get("attempts") or APPROVE_RETRY_MAX))
        )
        ...
        for index in range(1, APPROVE_RETRY_MAX + 1):
            ...
            last_error = (
                ...
                f"{response_error(approval_response, 500)}"
            )
    ...
    poll_config = enter_step("redirect_poll", "轮询 Stripe redirect")
    POLL_TIMEOUT = max(30, min(300, int(poll_config.get("timeout") or POLL_TIMEOUT)))
...
def run_single_seed_mode(token: str, proxy_seeds: list[str]) -> int:
    ...
    log(
        ...
        f"{CHECKOUT_COUNTRY} checkout/Bootstrap Stripe init -> "
...
# 独立脚本入口故意禁用；代理只能由系统的动态代理 API 分配器注入。
